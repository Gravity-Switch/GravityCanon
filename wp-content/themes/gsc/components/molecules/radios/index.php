...
gsc_define("radios", $defaults, function($data) {
  ...
  if ($data['content']['required']) {
    ...
    foreach ($data['content']['options'] as $radio) {
			$radio_checked = (!empty($radio['checked']) && $radio['checked']) ? "checked='checked'" : "";
			$radio_disabled = (!empty($radio['disabled']) && $radio['disabled']) ? "disabled='disabled'" : "";
			$radio_disabled_class = (!empty($radio['disabled']) && $radio['disabled']) ? "form__element--disabled" : "";
			$radios .= "
				<{$data['style']['item_container']} class='form__element element--radio {$radio_disabled_class}'>
					<input class='form__input input--radio' type='radio' {$radio_disabled} {$radio_checked} id='{$radio['id']}' name='{$data['style']['name']}' value='{$radio['value']}' />

					<label class='form__label label--radio' for='{$radio['id']}'>
						{$radio['text']}
					</label>
